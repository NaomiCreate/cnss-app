...
export class HistoryComponent implements OnInit {
    ...
    this.crudservice.get_userInfo().subscribe(data => {
        this.deviceID = data.map(c => {
          if (c.payload.doc.data()['is_device_owner'] == true){
            this.isOwner = true;
            console.log("device-id", c.payload.doc.data()['device-id']);
            return c.payload.doc.data()['device-id'];
          }
          else{
            this.isOwner = false;
            return "";
        ...
        console.log("device-id", this.deviceID[0]);
  ...
  get_alert_details(){
    ...
    .subscribe(data => {
      ...
      this.alertArray.sort((a, b) => {return a.timestamp-b.timestamp});
    })
  }
